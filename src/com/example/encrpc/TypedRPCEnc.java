		...
		if (m instanceof Let) {
			...
			if (mLet.getM1() instanceof App) {
				...
				if (app1.getFun() instanceof Lam && ((Lam) app1.getFun()).getLoc() == Location.Client) {
					...
					retEither.setLeft(new Let(mLet.getVal(), EncMain.substs(fLam.getTerm(), fLam.getStrArr(), app1.getArgs()), mLet.getM2()));
				...
				if (req1.getReq() instanceof Lam && ((Lam) req1.getReq()).getLoc() == Location.Server) {
					...
					retEither.setRight(new Pair<>(new ClientContext(Ctx.Load, mLet.getVal(), mLet.getM2()), new Let("r", new App(rLam, req1.getArgs()), new Var("r"))));
				...
				retEither.setLeft(new Let(let1.getVal(), let1.getM1(), new Let(mLet.getVal(), let1.getM2(), mLet.getM2())));
			...
			if (mLet.getM1() instanceof App) {
				...
				if (mApp1.getFun() instanceof Lam) {
					...
					if (fLam.getLoc() == Location.Server) {
						retEither.setRight(new Pair<>(ctx, new Let(mLet.getVal(), EncMain.substs(fLam.getTerm(), fLam.getStrArr(), mApp1.getArgs()), mLet.getM2())));
					}
				}
			}
			else if (mLet.getM1() instanceof Lam) {
				...
				retEither.setRight(new Pair<>(ctx, new Let(mLet1.getVal(), mLet1.getM1(), new Let(mLet.getVal(), mLet1.getM2(), mLet.getM2()))));
			}
