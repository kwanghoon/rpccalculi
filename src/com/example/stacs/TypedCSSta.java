...
public class TypedCSSta {
	...
	public static StaTerm repEvalServer(FunStore clientFS, FunStore serverFS, ClientContext clientCtx,
			ServerContext serverCtx, StaTerm m) {
		...
		if (m instanceof Let) {
			...
			if (m1 instanceof App) {
				...
				if (mApp1.getF() instanceof Clo) {
					...
					Pair<StaTerm, ServerContext> left = new Pair<>(new Let(mLet.getY(),
							CSStaMain.substs(CSStaMain.substs(closedFun.getM(), closedFun.getZs(), fClo.getVs()),
									closedFun.getXs(), mApp1.getWs()),
							mLet.getM2()), serverCtx);
				...
				if (mReq1.getF() instanceof Clo) {
					...
					TripleTup<ClientContext, ServerContext, StaTerm> right = new TripleTup<>(
							new ClientContext(new Context(new Ctx(mLet.getY(), mLet.getM2()))), serverCtx,
				...
				Pair<StaTerm, ServerContext> left = new Pair<>(CSStaMain.subst(mLet.getM2(), mLet.getY(), mClo1),
						serverCtx);
				either.getEither().setLeft(left);
				...
				Pair<StaTerm, ServerContext> left = new Pair<>(CSStaMain.subst(mLet.getM2(), mLet.getY(), mConst1),
						serverCtx);
				either.getEither().setLeft(left);
