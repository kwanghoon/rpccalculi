			...
			if (ttApp.getLoc() instanceof com.example.typedrpc.LocType) {
				...
				if (loc.getLoc() == Location.Client) {
					Let retLet = new Let(fvar, p1.getValue(),
							new Let(xvar, p2.getValue(), new Let(rvar, new App(f, xs), r)));
		...
		if (tt instanceof com.example.typedrpc.Const) {
			...
			if (ttApp.getLoc() instanceof com.example.typedrpc.LocType) {
				...
				if (loc.getLoc() == Location.Client) {
					...
					Lam commuteFun = new Lam(Location.Client, lamXs, new Let(yvar, new App(f, appXs), new Ret(y)));
					...
					Let retLet = new Let(fvar, p1.getValue(),
							new Let(xvar, p2.getValue(), new Let(rvar, new Call(commuteFun, callXs), r)));
					retPair = new Pair<>(p2.getKey(), retLet);
				}
				else {
					String fvar = "f" + i;
					...
					Let retLet = new Let(fvar, p1.getValue(),
							new Let(xvar, p2.getValue(), new Let(rvar, new App(f, appXs), r)));
